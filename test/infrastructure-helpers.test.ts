...
describe("Infrastructure files — always generated", () => {
  ...
  it("generates all helper files for a minimal service", async () => {
    ...
    expect(
      outputs["src/Generated/Internal/Utf8JsonBinaryContent.cs"],
    ).toBeDefined();
    expect(
      outputs["src/Generated/Internal/BinaryContentHelper.cs"],
    ).toBeDefined();
    expect(
      outputs["src/Generated/Internal/PipelineRequestHeadersExtensions.cs"],
    ).toBeDefined();
  });
...
describe("Utf8JsonBinaryContentFile", () => {
  /**
   * Verifies the file is generated at the correct path under
   * src/Generated/Internal/, matching the legacy emitter's output structure.
   */
  it("generates Utf8JsonBinaryContent.cs at the correct path", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    const key = Object.keys(outputs).find((k) =>
      k.endsWith("Internal/Utf8JsonBinaryContent.cs"),
    );
    expect(key).toBeDefined();
    expect(key).toBe("src/Generated/Internal/Utf8JsonBinaryContent.cs");
  });

  /**
   * Verifies the class declaration is correct: internal, partial, extends
   * BinaryContent. Not static because it's instantiated by BinaryContentHelper.
   */
  it("declares internal partial class extending BinaryContent", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    const content = outputs["src/Generated/Internal/Utf8JsonBinaryContent.cs"];
    expect(content).toContain(
  ...
  /**
   * Verifies the private readonly fields for stream and content are generated.
   * These fields hold the MemoryStream and inner BinaryContent instance.
   */
  it("contains private readonly fields", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    const content = outputs["src/Generated/Internal/Utf8JsonBinaryContent.cs"];
    expect(content).toContain("private readonly MemoryStream _stream;");
    expect(content).toContain("private readonly BinaryContent _content;");
  });

  /**
   * Verifies the JsonWriter auto-property is generated. This is used by
   * BinaryContentHelper to write JSON content via Utf8JsonWriter.
   */
  it("contains JsonWriter auto-property", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the parameterless constructor initializes all three members:
   * stream, content (via BinaryContent.Create), and writer.
   */
  it("contains parameterless constructor", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    const content = outputs["src/Generated/Internal/Utf8JsonBinaryContent.cs"];
    expect(content).toContain("public Utf8JsonBinaryContent()");
    expect(content).toContain("_stream = new MemoryStream();");
    expect(content).toContain("_content = BinaryContent.Create(_stream);");
    expect(content).toContain("JsonWriter = new Utf8JsonWriter(_stream);");
  });

  /**
   * Verifies the async and sync WriteToAsync/WriteTo override methods are
   * generated. These flush the JSON writer before forwarding to the inner content.
   */
  it("contains WriteToAsync and WriteTo overrides", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    ...
    expect(content).toContain("JsonWriter.Flush();");
  });

  /**
   * Verifies TryComputeLength override is generated. It returns the sum of
   * committed and pending bytes from the Utf8JsonWriter.
   */
  it("contains TryComputeLength override", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the Dispose override disposes all three resources in the
   * correct order: writer, content, stream.
   */
  it("contains Dispose override", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    ...
    expect(content).toContain("JsonWriter.Dispose();");
    expect(content).toContain("_content.Dispose();");
    expect(content).toContain("_stream.Dispose();");
  });
});

describe("BinaryContentHelperFile", () => {
  /**
   * Verifies the file is generated at the correct path under
   * src/Generated/Internal/, matching the legacy emitter's output structure.
   */
  it("generates BinaryContentHelper.cs at the correct path", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    const key = Object.keys(outputs).find((k) =>
      k.endsWith("Internal/BinaryContentHelper.cs"),
    );
    expect(key).toBeDefined();
    expect(key).toBe("src/Generated/Internal/BinaryContentHelper.cs");
  });

  /**
   * Verifies the class declaration is correct: internal, static, partial.
   * Static because all methods are factory methods, partial for extensibility.
   */
  it("declares internal static partial class", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
  ...
  /**
   * Verifies the generic FromEnumerable<T> method is generated. This converts
   * an IEnumerable<T> to a JSON array BinaryContent using WriteObjectValue.
   */
  it("contains FromEnumerable<T> method", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the BinaryData FromEnumerable overload with null handling and
   * NET6_0_OR_GREATER preprocessor directives for WriteRawValue support.
   */
  it("contains FromEnumerable(BinaryData) with #if directive", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    ...
    expect(content).toContain("content.JsonWriter.WriteNullValue();");
    expect(content).toContain("#if NET6_0_OR_GREATER");
    expect(content).toContain("content.JsonWriter.WriteRawValue(item);");
    expect(content).toContain("#endif");
  });

  /**
   * Verifies the ReadOnlySpan<T> overload is generated. This uses a
   * for-loop (not foreach) because ReadOnlySpan doesn't implement IEnumerable.
   */
  it("contains FromEnumerable<T>(ReadOnlySpan<T>) method", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    ...
    expect(content).toContain("for (; i < span.Length; i++)");
  });

  /**
   * Verifies the generic FromDictionary<T> method converts a dictionary
   * to a JSON object BinaryContent.
   */
  it("contains FromDictionary<T> method", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    ...
    expect(content).toContain("content.JsonWriter.WritePropertyName(item.Key)");
  });

  /**
   * Verifies the BinaryData FromDictionary overload with null handling and
   * NET6_0_OR_GREATER preprocessor directives.
   */
  it("contains FromDictionary(BinaryData) with #if directive", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the FromObject(object) method that serializes a single value
   * using WriteObjectValue with WireOptions.
   */
  it("contains FromObject(object) method", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the FromObject(BinaryData) overload with #if directive for
   * WriteRawValue vs JsonDocument.Parse fallback.
   */
  it("contains FromObject(BinaryData) with #if directive", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
...
describe("PipelineRequestHeadersExtensionsFile", () => {
  /**
   * Verifies the file is generated at the correct path under
   * src/Generated/Internal/, matching the legacy emitter's output structure.
   */
  it("generates PipelineRequestHeadersExtensions.cs at the correct path", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
    const key = Object.keys(outputs).find((k) =>
      k.endsWith("Internal/PipelineRequestHeadersExtensions.cs"),
    );
    expect(key).toBeDefined();
    expect(key).toBe(
      "src/Generated/Internal/PipelineRequestHeadersExtensions.cs",
    );
  });

  /**
   * Verifies the class declaration is correct: internal, static.
   * Static because all methods are extension methods.
   */
  it("declares internal static class", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the SetDelimited<T> extension method without format parameter is
   * generated. This joins enumerable values with a delimiter and sets the header.
   */
  it("contains SetDelimited<T> without format", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the SetDelimited<T> overload with SerializationFormat parameter
   * is generated. This supports formatted conversion (e.g., date formats).
   */
  it("contains SetDelimited<T> with format", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the Add extension method with prefix parameter is generated.
   * This adds multiple headers from a dictionary, prepending a prefix to
   * each header key. Used for header parameters with a common prefix.
   */
  it("contains Add with prefix method", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
      namespace TestService;
    `);
  ...
  /**
   * Verifies the required using directives are present for PipelineRequestHeaders,
   * IDictionary, and LINQ Select.
   */
  it("includes required using directives", async () => {
    const [{ outputs }] = await HttpTester.compileAndDiagnose(`
      @service
