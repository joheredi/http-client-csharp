...
namespace TestProjects.Spector.Tests.Http.Client.Structure.ClientOperationGroup
{
    public class ClientOperationGroupTests : SpectorTestBase
    {
        ...
        public void VerifyMethods()
        {
            ...
            var methodsOfSecondClient = typeof(SecondClient).GetMethods();
            ...
            Assert.IsNotNull(typeof(SecondClient).GetMethods().Where(m => m.Name == "FiveAsync").FirstOrDefault());
            //check existence of method to get the operation group client
            Assert.AreNotEqual(null, typeof(SecondClient).GetMethod("GetGroup5Client"));
        ...
        {
            var response = await new SecondClient(host, ClientType.ClientOperationGroup, null).FiveAsync();
            Assert.AreEqual(204, response.GetRawResponse().Status);
        });
        ...
        {
            var response = await new SecondClient(host, ClientType.ClientOperationGroup, null).GetGroup5Client().SixAsync();
            Assert.AreEqual(204, response.GetRawResponse().Status);
        });
    }
}
