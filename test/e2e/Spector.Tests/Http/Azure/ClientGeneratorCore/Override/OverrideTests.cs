...
namespace TestProjects.Spector.Tests.Http.Azure.ClientGeneratorCore.Override
{
    public class OverrideTests : SpectorTestBase
    {
        ...
        public Task Azure_ClientGenerator_Core_Override_GroupParameters() => Test(async (host) =>
        {
            ...
            // Test convenience method with individual parameters
            
            var response = await groupParameters.GroupAsync("param1", "param2");
            Assert.AreEqual(204, response.Status);

            // Verify that the convenience method exists with individual parameters
            Assert.IsNotNull(typeof(GroupParameters).GetMethod("GroupAsync", BindingFlags.Instance | BindingFlags.Public, new[] { typeof(string), typeof(string), typeof(CancellationToken) }));
            Assert.IsNotNull(typeof(GroupParameters).GetMethod("Group", BindingFlags.Instance | BindingFlags.Public, new[] { typeof(string), typeof(string), typeof(CancellationToken) }));
