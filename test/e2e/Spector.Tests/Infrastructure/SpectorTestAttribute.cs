...
namespace TestProjects.Spector.Tests
{
    ...
    internal partial class SpectorTestAttribute : TestAttribute, IApplyToTest
    {
        ...
        /// <summary>
        /// Resolves the generated code directory using RepoRoot from BuildPropertiesAttribute.
        /// Generated code lives at: {RepoRoot}/temp/e2e/Spector/http/{scenario}/src/Generated
        /// </summary>
        private static string GetGeneratedDirectory(Test test, bool kebabCaseDirectories = true)
        {
            var repoRoot = ((BuildPropertiesAttribute)typeof(TestServerBase).Assembly
                .GetCustomAttributes(typeof(BuildPropertiesAttribute), false)[0]).RepoRoot;

            // Extract namespace parts: TestProjects.Spector.Tests.Http.{Category}.{Scenario}.{TestClass}.{TestMethod}
            // Skip first 3 (TestProjects.Spector.Tests) and last 2 ({TestClass}.{TestMethod})
            var namespaceParts = test.FullName.Split('.').Skip(3);
            namespaceParts = namespaceParts.Take(namespaceParts.Count() - 2);

            var clientCodeDirectory = Path.Combine(repoRoot, "temp", "e2e", "Spector");
