...
/**
 * Props for the {@link ChangeTrackingDictionaryFile} component.
 */
export interface ChangeTrackingDictionaryFileProps {
...
/**
 * Generates the `ChangeTrackingDictionary.cs` internal helper class.
 *
 * This generic dictionary wrapper implements `IDictionary<TKey, TValue>` and
 * `IReadOnlyDictionary<TKey, TValue>` with lazy initialization and "undefined"
 * state tracking. It is used in deserialization code to distinguish between a
 * dictionary that was never set (undefined/absent from JSON) and an explicitly
 * empty dictionary.
 *
 * Key behavior:
 * - `IsUndefined` returns `true` when the inner dictionary has not been initialized
 * - Operations on an undefined dictionary return safe defaults (0, false, empty)
 * - `EnsureDictionary()` lazily initializes the inner dictionary on first mutation
 * - Explicit interface implementations for `IReadOnlyDictionary` Keys/Values
 *
 * The generated class matches the legacy emitter's `ChangeTrackingDictionaryDefinition`
 * output: `src/Generated/Internal/ChangeTrackingDictionary.cs`.
 */
export function ChangeTrackingDictionaryFile(
  props: ChangeTrackingDictionaryFileProps,
) {
  const header = getLicenseHeader(props.options);

  return (
    <SourceFile
      path="src/Generated/Internal/ChangeTrackingDictionary.cs"
