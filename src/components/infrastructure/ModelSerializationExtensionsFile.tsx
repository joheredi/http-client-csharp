 ...
 */
export interface ModelSerializationExtensionsFileProps {
...
/**
 * Generates the `ModelSerializationExtensions.cs` internal static helper class.
 *
 * This class provides:
 * - `WireOptions` — the default `ModelReaderWriterOptions` with `"W"` (wire) format
 *   used throughout serialization/deserialization code.
 * - `JsonDocumentOptions` — configured `JsonDocumentOptions` with `MaxDepth = 256`
 *   used when parsing JSON responses.
 * - Extension methods for `JsonElement`, `Utf8JsonWriter`, `XElement`, and `XmlWriter`
 *   that handle type conversions, date formatting, Base64 encoding, and recursive
 *   object writing for both JSON and XML serialization.
 *
