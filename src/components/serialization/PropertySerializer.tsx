...
/**
 * Maps a TCGC numeric scalar kind to the .NET framework type name used in
 * the `ToSerial{FrameworkName}()` method generated by LiteralTypeSerializationFile.
 *
 * @param kind - The TCGC scalar kind (e.g., "float32", "int32").
 * @returns The .NET framework type name (e.g., "Single", "Int32").
 */
function getLiteralSerialMethodSuffix(kind: string): string {
  switch (kind) {
    case "float32":
      return "Single";
    case "float64":
...
/**
 * Returns a value transform function for literal wrapper struct types.
 *
 * Literal wrapper structs (e.g., FloatLiteralPropertyProperty) store the
 * underlying value in a private `_value` field. To serialize them to JSON,
 * numeric types need `.ToSerial{FrameworkName}()` to extract the value,
 * and string types need `.ToString()`.
 *
 * @param property - The model property to check.
 * @returns A value transform function, or `undefined` if no transform is needed.
 */
function getLiteralWrapperValueTransform(
  property: SdkModelPropertyType,
  ...
  const valueTypeKind = (unwrapped as SdkConstantType).valueType.kind;
  if (valueTypeKind === "string") {
    return (name) => `${name}.ToString()`;
  }
  const suffix = getLiteralSerialMethodSuffix(valueTypeKind);
  return (name) => `${name}.ToSerial${suffix}()`;
}
...
export function WritePropertySerialization(
  ...
  // For literal wrapper struct properties, the property accessor returns the
  // wrapper struct (e.g., FloatLiteralPropertyProperty), not the underlying
  // primitive. Apply a value transform to extract the raw value for JSON writing.
  const literalTransform = getLiteralWrapperValueTransform(property);
  if (literalTransform && !writeInfo.valueTransform) {
    writeInfo.valueTransform = literalTransform;
  }
